...
- 无结构：流式的存储所有的 words/bytes，UNIX 就定义所有文件就是普通的一串字节；
...
???+ section "位图"
    ...
    使用这种方法需要带来的额外开销是，有多少 blocks 就需要多少个额外的 bits。例如，假设块大小为 4 KiB，硬盘一共有 1 GiB，则需要 $2^30 / 2^12 = 2^18 = 256 KiB$ 的**额外空间**来存储位图，也就是$256 KiB / 4 KiB = 64$ 个 block。
